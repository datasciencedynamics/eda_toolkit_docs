...
.. _Version 0.0.26: https://lshpaner.github.io/eda_toolkit_docs/v0.0.26/index.html

Summary
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Adds three enhancements to ``generate_table1`` to improve output correctness and
usability, driven by reviewer feedback on a clinical Table 1. Also removes locally
redefined ``_flag_iqr`` and ``_flag_zscore`` helpers from inside ``detect_outliers``
that were silently shadowing module-level imports.

Motivation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

During peer review of a manuscript using ``generate_table1`` output, several issues
were identified:

- The Overall column for continuous variables was displaying sample counts rather than
  mean (SD), because the Mean and SD columns were being dropped before any combined
  Overall representation was constructed.
- The Overall column for categorical variables was missing percentage values for
  subcategory rows entirely.
- Categorical parent-level group columns (e.g., No Rapid Decline and Rapid Decline)
  were displaying raw counts without percentages, while subcategory rows correctly
  showed ``n (%)``.
- There was no way to control whether subcategory proportions were expressed relative
  to the full cohort or relative to the non-missing count of that variable.

Additionally, ``_flag_iqr`` and ``_flag_zscore`` were already imported from
``_data_manager_utils`` at the top of the file. Local redefinitions inside
``detect_outliers`` were shadowing those imports without any behavioral difference.

Changes
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

1. ``include_overall`` parameter (default: ``True``)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

A new ``include_overall: bool = True`` parameter has been added to the function
signature. When ``True``, an Overall column is inserted as the second column in both
``df_continuous`` and ``df_categorical``, positioned immediately after the Variable
column.

- For continuous variables, Overall shows ``mean (SD)`` computed on the full cohort,
  e.g. ``58.06 (16.99)``.
- For categorical parent rows (e.g., ``sex``, ``race_ethnicity_cat``), Overall shows
  the total non-missing count and its proportion relative to the cohort, e.g.
  ``1,187,310 (100.00%)``.
- For categorical subcategory rows (e.g., ``sex = Female``), Overall shows the
  subcategory count and its proportion, e.g. ``659,426 (55.54%)``.

The Overall column is computed in a ``make_overall`` helper function that runs against
the raw ``continuous_parts`` and ``categorical_parts`` lists before ``pd.DataFrame()``
is called. This ensures that Mean and SD are still available as raw floats at the time
Overall is constructed, enabling clean ``f"{mean:.2f} ({sd:.2f})"`` formatting without
parsing already-formatted strings.

After ``format_numeric_cols`` runs, the Overall column is inserted via
``df.insert(1, "Overall", overall_values)``. A guard was added to check
``if not df_continuous.empty`` and ``if not df_categorical.empty`` before calling
``insert``, preventing an ``IndexError`` on Python 3.8 when one DataFrame has zero
rows.

Setting ``include_overall=False`` restores the previous output exactly, so all
existing call sites are unaffected.

2. ``proportion_relative_to_cohort`` parameter (default: ``True``)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

A new ``proportion_relative_to_cohort: bool = True`` parameter controls the
denominator used when computing ``Proportion (%)`` for categorical subcategory rows
(i.e., rows generated by the ``value_counts=True`` path).

- When ``True`` (default), the denominator is ``total_rows``, representing the full
  cohort size regardless of missingness. This preserves existing behavior and means
  proportions across subcategories of a complete variable sum to 100%.
- When ``False``, the denominator is ``series.notna().sum()``, the number of
  non-missing observations for that variable. This is more interpretable when a
  variable has substantial missingness, since subcategory proportions then reflect the
  distribution among patients with a recorded value rather than being diluted by the
  missing fraction.

This parameter applies only to subcategory rows produced by the ``value_counts`` block.
Parent-level categorical rows and all continuous rows always use ``total_rows`` as
their denominator.

3. Categorical group column formatting fix
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

When ``groupby_col`` is specified, the parent-level categorical summary row now shows
``n (%)`` in each group column rather than a raw count. Previously::
...
The percentage denominator is the group size computed as
``(df[groupby_col] == g).sum()``, giving the correct within-group percentage for the
parent variable row.

Additionally, ``group1_label`` and ``group2_label`` were removed from ``int_cols``
inside ``format_numeric_cols``. Previously these were included in the integer
formatting pass, which caused the newly formatted ``n (%)`` strings to be blanked out
since they could not be cast to integer. The group label columns now pass through
``format_numeric_cols`` untouched.

4. Removed duplicate helpers from ``detect_outliers``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

- Deleted local ``_flag_iqr`` and ``_flag_zscore`` definitions from inside
  ``detect_outliers``.
- Both functions are already imported from ``_data_manager_utils`` at the top of the
  file; the local copies were silently shadowing those imports.
- No behavioral change.

Testing
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- Updated ``test_table1_return_markdown_only`` to reflect the new column order.
  The assertion was changed from ``"| Variable | Type | Mode"`` to
  ``"| Variable | Overall | Type | Mode"``.
...
.. code-block:: text
    ...
    src/eda_toolkit/_plot_utils.py             135     23    83%
    src/eda_toolkit/art.py                      54      3    94%
    src/eda_toolkit/data_manager.py            855     95    89%
    src/eda_toolkit/plots.py                  1203    233    81%
    ------------------------------------------------------------
    TOTAL                                     2294    356    84%
...
All changes are fully backward compatible. ``include_overall=True`` and
``proportion_relative_to_cohort=True`` are the defaults, so existing call sites receive
the improved behavior automatically. Users who want the previous output can pass
``include_overall=False``. The group column formatting fix corrects a bug rather than
changing an intentional behavior, and the helper removal is a no-op at runtime.
